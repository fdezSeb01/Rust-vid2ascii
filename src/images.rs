use image::{io::Reader, GenericImageView, GrayImage, ImageError};
...
pub fn find_image_from_path(path: String) -> Result<(u32, u32, GrayImage), ImageDataError> {
    match Reader::open(&path) {
        //hace match de si la promesa de result es Ok prosigue, si es error manda el error
        Ok(image_reader) => {
            match image_reader.format() {
                //como format regresa un Option (Some o none), hace el caso de que si regrese algo o else si no
                Some(_d) => {
                    match image_reader.decode() {
                        //otra vez un match para que regrese algo si es Ok o error si no
                        Ok(image) => {
                            Ok((image.dimensions().0, image.dimensions().1, image.to_luma8()))
                        } //la regresa como escala de grises
